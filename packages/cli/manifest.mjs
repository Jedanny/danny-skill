...
export function parsePackagingIncludeList(content, profile) {
  const manifest = parseManifest(content);
  return manifest.packaging?.[profile]?.include ?? null;
}

export function parseValidationSpec(content) {
  const manifest = parseManifest(content);
  const validation = manifest.validation ?? {};
  return {
    requiredFiles: validation.required_files ?? [],
    jsonFiles: validation.json_files ?? [],
    markdownContracts: (validation.markdown_contracts ?? []).map((contract) => ({
      path: contract.path,
      mustContain: contract.must_contain ?? [],
    })),
  };
}
